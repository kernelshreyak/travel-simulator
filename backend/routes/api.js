...
router.get("/geocode", (req, res) => {
  ...
  axios
    ...
    .then((resp) => {
      ...
      axios
        .get(
          `https://api.openweathermap.org/data/2.5/weather?lat=${locationpoint.lat}&lon=${locationpoint.lng}&appid=${process.env.OPENWEATHERMAP_KEY}`
        )
        .then((resp2) => {
          res.json({
            status: "success",
...
router.get("/get_route", (req, res) => {
  ...
  if (vehicle == "car") {
    ...
    try {
      ...
      res.json({
        status: "success",
        points: findTrainRoute(
          departureStation.latitude_deg,
          departureStation.longitude_deg,
          arrivalStation.latitude_deg,
          arrivalStation.longitude_deg
        ),
        // [
    ...
    try {
      ...
      const departureAirport = _.find(allAirPorts, function (airport) {
        ...
        return (
          d <= 50 &&
          (airport.type == "medium_airport" || airport.type == "large_airport")
        );
      });
      ...
      const waypoint_gap = 10; //km
      const airway_points = generateAirWayPoints(
        departureAirport.latitude_deg,
        departureAirport.longitude_deg,
        arrivalAirport.latitude_deg,
        arrivalAirport.longitude_deg,
        waypoint_gap
      );
